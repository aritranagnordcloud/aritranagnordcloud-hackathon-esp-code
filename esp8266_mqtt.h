...
void messageReceivedAdvanced(MQTTClient *client, char topic[], char bytes[], int length)
{
  if (length > 0){
    ...
    display.init();
    String result = "URL:-"+ String(bytes) + "\n";
    display.drawStringMaxWidth(0, 0, 130,result);
    Serial.printf("Screen write done");
    display.display();
  } else {
...
void setupCloudIoT()
{
  ...
  mqttClient->subscribe("/devices/"+ String(device_id) +"/events",0);
}
